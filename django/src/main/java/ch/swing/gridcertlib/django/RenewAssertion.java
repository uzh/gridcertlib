...
public class RenewAssertion extends HttpServlet
{
        ...
        throws ServletException, IOException
    {
        ...
        ctx_.log("RenewAssertion: got request from referer '" + decodeUrl(encodedDestinationUrl) + "'");
        ...
        if (action.equals("do")) {
            ...
            // "done".  (Incidentally, this is the main reason for
            // using this strange encoding of the requests in the URL:
            // we cannot pass a URL with a query part as argument to
            // the `return` parameter, yet we need to know what is the
            // original return location -- therefore we use a return
            // URL that has it appended as part of the path info.)
            final String redirectUrl = logoutUrl_ + "?return=" 
                + selfUrl_ + "/done/" + encodedDestinationUrl;
            ...
            final String decodedDestinationUrl = decodeUrl(encodedDestinationUrl);
            ctx_.log("RenewAssertion: returning client, redirect to " + decodedDestinationUrl);
            response.sendRedirect(response.encodeRedirectURL(decodedDestinationUrl));
        }
        else
        ...
        throws ServletException
    {
        ...
        // return renewAssertionUrl + "/do/" 
        //     + URLEncoder.encode(URLEncoder.encode(destinationUrl, "UTF-8"), "UTF-8");
    }
        ...
        throws ServletException
    {
        try {
            ...
            //return URLEncoder.encode(URLEncoder.encode(decoded, "UTF-8"), "UTF-8");
        }
        catch (UnsupportedEncodingException ex) {
            throw new ServletException("RenewAssertion.encodeUrl: "
                                       + "Cannot encode string '" + decoded 
                                       + "' with UTF-8:" + ex.getMessage());
        }
    }
        ...
        throws ServletException
    {
        try { 
            ...
            //return URLDecoder.decode(URLDecoder.decode(encoded, "UTF-8"), "UTF-8");
        }
        catch (UnsupportedEncodingException ex) {
            throw new ServletException("RenewAssertion.encodeUrl: "
                                       + "Cannot decode string '" + encoded 
                                       + "' with UTF-8:"
                                       + ex.getMessage());
        }
    }
